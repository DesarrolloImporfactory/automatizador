...
function sendWhatsappMessage($user_info, $template_name, $template_parameters, $config) {
    $config['whatsapp_id'] = "350785748112507";
    $config['token'] = "EAAeFJ9JZBf40BOxpyvllsJMmZBO7WsSwLCoLviaxQM8uT7EXJ5M6dWfcZBAOT7kgpuTYiq58HZCdZC2h3MhN5ODzahFvEQxuYFUGcy982ZAkKFZAlkernQ3FzjogE0eF9EfaCVW7msU1NqiVZCME1WTksav6rG74lhKqyg5ZBafMvrGBPWFZBMaAlwfkeTUeCasye5rXE7LDmCbY4FSK0dYHcZD";

    $url = 'https://graph.facebook.com/v11.0/' . $config['whatsapp_id'] . '/messages';
    $token = $config['token'];
...
function insertInteractions($conn, $block_details, $id_automatizador, $user_id, $data) {
    foreach ($block_details as $block) {
        ...
        if ($block['block_sql_data']['tipo'] == 8) {
            $respuesta_accion = sendWhatsappMessage($data['user_info'], 'template_name', ['param1', 'param2'], []);
