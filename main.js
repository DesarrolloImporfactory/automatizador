document.addEventListener("DOMContentLoaded", function () {
  ...
  function createBlockElementActive(block) {
    ...
    var blockyleftDiv =
      "<div class='blockyleft'><i class='" +
      block.icon +
      " text-primary fa-2x'></i><p class='blockyname'>" +
      block.name +
      "</p></div>";
    ...
    var blockyinfoDiv =
      "<div class='blockyinfo px-2'><p>" + block.description + "</p></div>";
  ...
  function snapping(drag, first) {
    ...
    var blockelemtype =
      blocks_type[parseInt(drag.querySelector(".blockelemtype").value) - 1];
    //7
    console.log(JSON.stringify(blockelemtype));

    if (
      first == true &&
      (blockelemtype["category"] == "Disparadores" ||
        blockelemtype["category"] == "Condiciones")
    ) {
      drag.innerHTML += createBlockElementActive(blockelemtype);
      //console.log("first: "+first);
      return true;
    } else if (first == false && blockelemtype["category"] == "Condiciones") {
      drag.innerHTML += createBlockElementActive(blockelemtype);
      //console.log("first: "+first);
      return true;
    } else if (first == false && blockelemtype["category"] != "Disparadores") {
      drag.innerHTML += createBlockElementActive(blockelemtype);
      //console.log("first: "+first);
      return true;
    } else {
  ...
  document.getElementById("removeblock").addEventListener("click", function () {
    ...
    window.location.href =
      "tabla_automatizadores.php?id_configuracion=" + window.id_configuracion;
  });
  ...
  if (window.flowlyOutputBlocks) {
    function generateHTML(jsonInput) {
      ...
      const blockElements = inputObj.blocks
        .map((block) => {
          const blockType = blocks_type.find(
            (bt) =>
              bt.value ==
              block.data.find((data) => data.name === "blockelemtype").value
          );
          if (!blockType) {
            console.error("Block type not found for block:", block);
            return "";
          }
          const blockStyle = block.attr.find((attr) => attr.style)
            ? block.attr.find((attr) => attr.style).style
            : "";
          const blockElemTypeValue = block.data.find(
            (data) => data.name === "blockelemtype"
          )
            ? block.data.find((data) => data.name === "blockelemtype").value
            : "";
          const blockIdValue = block.data.find(
            (data) => data.name === "blockid"
          )
            ? block.data.find((data) => data.name === "blockid").value
            : "";

          const indicator =
            block.id === 0
              ? `<div class="indicator invisible" style="left: 154px; top: 134px;"></div>`
              : "";
      ...
      const arrowElements = inputObj.blockarr
        .map((block, index) => {
          if (block.parent !== -1) {
            const parentBlock = inputObj.blockarr.find(
              (b) => b.id === block.parent
            );
            if (!parentBlock) {
            ...
            const arrowPath = `M${parentCenterX - arrowLeft} 0L${
              parentCenterX - arrowLeft
            } ${arrowHeight / 2}L${childCenterX - arrowLeft} ${
              arrowHeight / 2
            }L${childCenterX - arrowLeft} ${arrowHeight}`;
            ...
            return `<div class="arrowblock" style="left: ${arrowLeft}px; top: ${parentBottomY}px; width: ${arrowWidth}px; height: ${arrowHeight}px; visibility: hidden;">
                                ...
                                <svg preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <path d="M${childCenterX - arrowLeft - 5} ${
              arrowHeight - 5
            }H${childCenterX - arrowLeft + 5}L${
              childCenterX - arrowLeft
            } ${arrowHeight}L${childCenterX - arrowLeft - 5} ${
              arrowHeight - 5
            }Z" fill="#C5CCD0"></path>
                                </svg>
                            </div>`;
  ...
  var doneTouch = function (event) {
    ...
    if (event.type === "mouseup" && aclick && !noinfo) {
      ...
      if (
        ...
        var valorBlockID = tempblock.querySelector(
          'input[name="blockid"]'
        ).value;
        // blockelemtype
        var valorTipoID = tempblock.querySelector(
          'input[name="blockelemtype"]'
        ).value;
        ...
        console.log(
          "El resultado del anterior formulario es: " +
            JSON.stringify(formDataByBlock[valorBlockID])
        );

        var codigoHTML = `
                        <h4 id="header2">${
                          blocks_type[valorTipoID - 1]["name"]
                        }</h4>
        ...
        if (valorTipoID == "8") {
          const selectedOptions =
            formDataByBlock[valorBlockID]?.id_whatsapp_message_template || "";
          ...
          codigoHTML += `
                            ...
                            <div class="form-group col-12 p-2">
                                ...
                                <textarea class="form-control" id="mensaje" name="mensaje" rows="3" placeholder="Mensaje">${
                                  formDataByBlock[valorBlockID]?.mensaje || ""
                                }</textarea>
        ...
        setTimeout(function () {
          // Asunto
          var selectInsertAsunto =
            document.getElementById("selectInsertAsunto");
          ...
          // Mensaje
          var selectInsertMensaje = document.getElementById(
            "selectInsertMensaje"
          );
        ...
        if (valorTipoID == "10") {
          codigoHTML += `
                            <div class="form-group col-12 p-2">
                                ...
                                <input type="text" class="form-control" id="texto_recibir" name="texto_recibir" placeholder="Condicion de Texto a Recibir" value="${
                                  formDataByBlock[valorBlockID]
                                    ?.texto_recibir || ""
                                }">
                            </div>
