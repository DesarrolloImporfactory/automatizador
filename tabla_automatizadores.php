...
if (!$id_configuracion) {
    // Redirige a tabla_configuraciones.php si no se proporciona id_configuracion
...
const automatizadoresFields = [
    { "nombre_campo_mysql": "nombre", "nombre_campo_visible": "Nombre", "tipo_campo": "text", "requerido": true, "aparece_en_tabla": true, "seccion_modal": "Información General", "class_input_group": "col-12 col-md-12" },
    ...
    { "nombre_campo_mysql": "estado", "nombre_campo_visible": "Estado", "tipo_campo": "checkbox", "requerido": false, "aparece_en_tabla": true, "seccion_modal": "Información General", "class_input_group": "col-12 col-md-6" }
];
...
$(document).ready(function() {
    ...
    $('#automatizadoresForm').submit(function(event) {
        ...
        let formData = { id: $('#automatizadorId').val(), action: $('#automatizadorId').val() ? 'update' : 'create' };
        automatizadoresFields.forEach(field => {
            if (field.tipo_campo === 'checkbox') {
...
function loadAutomatizadores() {
    $.ajax({
        url: '<?php echo $url_crud; ?>',
        type: 'POST',
        data: { action: 'read', id_configuracion: idConfiguracion },
        success: function(response) {
